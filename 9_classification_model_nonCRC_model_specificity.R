...
if (TRUE){
  option_list = list(
    ...
    make_option(c("-m", "--metadata"), type="character", default="nonCRC_meta.csv",
                help="metadata file [default %default]"),
    make_option(c("-f", "--feature_profile"), type="character", default="Profile/NonCRC_marker_vali_table_ra.csv",
...
feature <- rownames(feat.all)
#print(feature)
feat.all[is.na(feat.all)] <- 0
rownames(meta.all) <- meta.all$Run
meta.all$StudyID <- factor(meta.all$dataset)
feat.sig <- t(feat.all[,meta.all$Run])
...
sites <- c('PRJEB17784','PRJEB1786','PRJEB6337')
for (s in sites){
  print(s)
  meta.s <- filter(meta.all, dataset==s)
  data.raw <- data_use[meta.s$Run,]
  data.s <- ovun.sample(Group ~ ., data = data.raw, method = "under", seed = 2021)$data
  ...
  for (i in (1:20)){
    ...
    write.csv(s_matrix,paste(opts$workplace,"NonCRC",opts$output,"_selfcv_repeat_metric_",s,".csv",sep=''))
    write.csv(s_matrix_max,paste(opts$workplace,"NonCRC_",opts$output,"_selfcv_repeat_max_",s,".csv",sep=''))
    write.csv(model_s,paste(opts$workplace,"NonCRC_",opts$output,"_selfcv_repeat_max_para_",s,".csv",sep=''))
  }
}
